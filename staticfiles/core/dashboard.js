(function () {
  ...
  document.addEventListener('click', function (ev) {
    ...
    // ✅ Fixed line: always post to /dashboard/login-time/
    ...
    fetch(action, {
      ...
      .then(data => {
        if (data && data.ok && data.attendance) {
          ...
          if (att.logout_time && att.login_time) {
            btn.innerText = 'Login';
            feedback.innerText = `Logged out at ${new Date(att.logout_time).toLocaleString()}`;
          } else if (att.login_time && !att.logout_time) {
            btn.innerText = 'Logout';
            feedback.innerText = `Logged in at ${new Date(att.login_time).toLocaleString()}`;
          } else {
            feedback.innerText = 'Attendance recorded';
          }
          // Optionally reload to update other widgets
      ...
      .catch(err => {
        ...
        feedback.innerText = err.message || 'Could not record attendance';
      })
  ...
  document.addEventListener('submit', function (ev) {
    ...
    if (!form.classList.contains('ajax-form')) return; // ignore non-ajax forms
    ev.preventDefault();
